...
function draw_grid()
  ...
  -- Store computed values for phase 2
  cfg.year_margin = year_margin
  cfg.box_width = box_width
  cfg.week_num_space = week_num_space

  -- Measure actual header height by creating a temporary box, then call phase 2
  local header_content = string.format([[{\hbox to %.5fin{\hfil %s 52\hfil}}]], week_num_space, c.week_num_size)
  tex.sprint([[\setbox0=\hbox{]] .. header_content .. [[}\xdef\headerheight{\the\ht0}]])
  tex.sprint([[\directlua{draw_grid_phase2()}]])
end

function draw_grid_phase2()
  local c = cfg -- shorthand

  -- Get measured header height from box0 (in scaled points, 65536 sp = 1pt)
  local header_height_sp = tex.box[0].height
  local header_height_pt = header_height_sp / 65536
  ...
  texio.write_nl(string.format('**********************************WIDTH: %.3f - HEIGHT: %.3f', c.box_width, box_height))

  -- Print week numbers across the top
  local week_num_start = c.year_margin + c.box_width/10
  tex.print(string.format([[\noindent\kern%.5fin]], week_num_start))
  for week = 1, 52 do
    tex.print(string.format([[{\hbox to %.5fin{\hfil %s %d\hfil}}]], c.week_num_space, c.week_num_size, week))
  end
  ...
  for year = 0, c.years do
    ...
    for week = 1, 52 do
      ...
      tex.print(string.format([[\fbox{%s\vbox to %.5fin{\vfil\hbox to %.5fin{\hfil}\vfil}}]],
           date_string, box_height, c.box_width))
    end
